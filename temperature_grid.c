...
void set_bgmap(grid_model_t *model, layer_t *layer)
{
	...
	for(u=0; u < layer->flp->n_units; u++) {
		...
		i1 = MAX(i1, 0);
		j1 = MAX(j1, 0);
		i2 = MIN(i2, model->rows);
		j2 = MIN(j2, model->cols);

		// JOHANN dropped sanity check for block-grid matching, may occur easily
		// for softblocks w/ floating point coordinates; simply avoid such
		// problems by considering grid constraints, see above
//		/* sanity check	*/
//		if((i1 < 0) || (j1 < 0)) {
						...
						grid_model_vector_t *temp)
{
	...
	double **sum;
	//BU_3D: 
	int k;   /*Used for iterating though the units to compute area,total area and weighted area */
	double area,total_area,warea=0.0;  /*used to calculate weighted mean for rz */
	double w_rz=0.0,temp_rz;   /*weighted mean of rz for the layer*/
	int i1, j1;   /* used to index into det3D_grid_Values to retrieve the rz value for a specific unit */
	//end->BU_3D
	...
	/* last layer	*/
		...
		for(n=nl-2; n >= 0; n--) {
			 ...
			 * However, when Heterogenous R-C assignment (detailed_3D_used) is on, each unit might have a different rz value.
			 * Therefore, we use a weighted rz value based on the areas and rz values of each unit in the layer.
			 */
			if(model->config.detailed_3D_used){
				total_area= get_total_area(model->layers[n].flp);
				for(k=0;k<model->layers[n].flp->n_units;k++){
					area = (model->layers[n].flp->units[k].height*model->layers[n].flp->units[k].width);//Area of Unit
					warea = area/total_area;// Weighted Area = Area of Unit/Area of Layer
